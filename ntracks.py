...
func.SetParameters(100000,1116,2.3, 100000, 1, -140700,130) 
f1 = ROOT.TF1("f1", signal_2, 1090, 1140)
func.SetParLimits(5,-1000000,1000000)
hist = TH1D("Lambda_MASS", "Lambda_MASS", 50,1090,1140)
hist.SetTitle("No. of #Lambda_{0} Candidates as a Function of Their Mass")
X = hist.GetXaxis()
X.SetTitle("Invariant Mass p#pi(#frac{MeV}{c^{2}})")
Y = hist.GetYaxis()
Y.SetTitle("# #Lambda_{0} with 30#leqnTracks<120(#frac{MeV}{c^{2}})")
for i in range(entries):
    MyTree.GetEntry(i)
    if 30 <= MyTree.nTracks < 120 and MyTree.Lambda_ID > 0:
