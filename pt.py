...
func = ROOT.TF1("func", combined, 1100,1140) 
func.SetParameters(70000,1116,2.7,70000,1, -155000,150) 
f1 = ROOT.TF1("f1", signal_2, 1100,1140)
func.SetParLimits(5,-1000000,1000000)
hist = TH1D("Lambda_MASS", "Lambda_MASS", 40 ,1100,1140)
hist.SetTitle("No. of #bar{#Lambda_{0}} Candidates as a Function of Their Mass")
X = hist.GetXaxis()
X.SetTitle("Invariant Mass p#pi(#frac{MeV}{c^{2}})")
Y = hist.GetYaxis()
Y.SetTitle("# #bar{#Lambda_{0}} with 300#leqp_{t}<850 (#frac{MeV}{c})")
for i in range(entries):
    MyTree.GetEntry(i)
    if 300<= MyTree.Lambda_PT < 850 and MyTree.Lambda_ID < 0:
